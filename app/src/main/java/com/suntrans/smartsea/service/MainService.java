package com.suntrans.smartsea.service;
...
public class MainService extends Service {
    ...
    public int sixSensorPort=8271;    //服务器端口
                ...
                new Thread()   //新建子线程，发送命令
                {
                    ...
                    public void run(){
                        ...
                        try
                        {
                            ...
                            String str="";
                            if(dev==0)
                                    str=toServer+Converts.GetCRC(bt, dev, bt.length);
                            str= str.replace(" ","");
                            byte[] bt1=Converts.HexString2Bytes(str);      //将完整的命令转换成十六进制
                            if(!client.isClosed())
                            ...
                            try
                            {
                                ...
                                out = new DataOutputStream(client.getOutputStream());
                                Thread.sleep(100);
                                String toServer = "";  //发送手机地址，发送后手机才能接收到服务器的数据
                                toServer = toServer.replace(" ", "");    //去掉空格
                                byte[] bt = null;
                                bt = Converts.HexString2Bytes(toServer);
                                String str = toServer + Converts.GetCRC(bt, 0, bt.length);   //添加校验码
...
//                                }
                                ...
                                str=toServer+Converts.GetCRC(bt, 0, bt.length);   //添加校验码和包尾
