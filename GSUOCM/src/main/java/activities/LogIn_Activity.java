...
import static WebServices.soap.Inquiry_UserInfo;
            ...
            { 
	            ...
	            {	            		    
					...
					new Thread(){
						@Override
						public void run() {
							SoapObject result = Inquiry_UserInfo(UserID);
	            ...
	            else    //用户是楼长、宿管、中心管理员等
	            {
	            	...
	            	intent.setClass(LogIn_Activity.this, AllRoom_Activity.class);//设置要跳转的activity
	            	startActivity(intent);//开始跳转
	            	overridePendingTransition(R.anim.slide_in_right, R.anim.slide_out_left);//设置activity切换效果
	            	finish();
	            }
...
//								Intent intent=new Intent();
//								intent.putExtra("UserID", UserID);
//								intent.putExtra("Role", 6);  //角色号
//								intent.setClass(LogIn_Activity.this, AllRoom_Activity.class);//设置要跳转的activity
//								startActivity(intent);//开始跳转
//								overridePendingTransition(R.anim.slide_in_right, R.anim.slide_out_left);//设置activity切换效果
//								finish();
								SoapObject object = soap.Inquiry_UserInfo("1");
								if(object==null){
									System.out.println("sbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb");
								}else {
									System.out.println("sccccccccccccccccccccccccccccccccccccccc");
								}
							}
							catch(Exception e){
								Message msg=new Message();
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
			Converts.setTranslucentStatus(LogIn_Activity.this,true);
		}
	}
